...
public class ProposalPage extends abstractWebPage {
    ...
    public void makeProposalActive() {
        List<String> proposals= new ArrayList<String>();
        List<WebElement> elements= VoyantaDriver.getCurrentDriver().findElements(By.className("mp_item"));

        for(WebElement element: elements){
                if(element.getText().contains(proposal_name)){
                        LOGGER.info("Make proposal active");
                        element.findElement(By.className("toggleActivation")).click();
                        VUtils.accept_alert();
                }
